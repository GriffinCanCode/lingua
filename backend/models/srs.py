...
class SyntacticPattern(Base):
    ...
    language = Column(String(10), nullable=False, default="ru")
    features = Column(JSONB, nullable=False)  # UD features: case, number, tense, etc.
...
class SentencePattern(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    sentence_id = Column(UUID(as_uuid=True), ForeignKey("sentences.id", ondelete="CASCADE"), nullable=False)
    pattern_id = Column(UUID(as_uuid=True), ForeignKey("syntactic_patterns.id", ondelete="CASCADE"), nullable=False)
