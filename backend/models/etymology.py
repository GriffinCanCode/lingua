...
class EtymologyNode(Base):
    ...
    extra_data = Column(JSONB, default=dict)  # Additional properties
...
class EtymologyRelation(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    source_id = Column(UUID(as_uuid=True), ForeignKey("etymology_nodes.id", ondelete="CASCADE"), nullable=False)
    target_id = Column(UUID(as_uuid=True), ForeignKey("etymology_nodes.id", ondelete="CASCADE"), nullable=False)
